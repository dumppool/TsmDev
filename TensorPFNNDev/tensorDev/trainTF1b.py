...
def PFNNetA(X_nn, keep_prob, reuse_variables):
  #with tf.variable_scope('PfDevNet', reuse = reuse):
  #with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
     ####parameter of nn 
     # b是又一个Softmax模型的参数，我们一般叫做“偏置项”（bias）
     W0 = tf.get_variable("weight", shape=(343, 256))#tf.zeros([343, 256]))
     b0 = tf.get_variable("b0",shape=(256))#tf.Variable(tf.zeros([256]))
     H0 = tf.matmul(X_nn, W0) + b0
     
     W1 = tf.get_variable("weight1",shape=(256, 311))#tf.Variable(tf.zeros([256, 311]))
     b1 = tf.get_variable("b1",shape=(311))#tf.Variable(tf.zeros([311]))
     H1 = tf.matmul(H0, W1) + b1
     # y=softmax(Wx + b)，y表示模型的输出
     return H1
