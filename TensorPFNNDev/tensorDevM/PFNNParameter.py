...
class PFNNParameter:
    def __init__(self, shape, rng, phase, name):
        ...
        """"alpha and beta"""
        #self.weight_shape (4, 512, 342)
        #self.bias_shape (4, 512)
        self.alpha        =  tf.get_variable(name=name+'alpha', shape= self.weight_shape ) #tf.Variable(self.initial_alpha(rng), name=name+'alpha') 
        self.beta         =  tf.get_variable(name=name+'beta' , shape= self.bias_shape   )#tf.Variable(self.initial_beta(), name=name+'beta') 
